...
namespace VRTK
{
    ...
    public class SDK_InputSimulator : MonoBehaviour
    {
        ...
        {
            if (Input.GetKeyDown(triggerAlias))
            {
                GameObject hand = null;
                SDK_BaseController.ControllerHand handType = SDK_BaseController.ControllerHand.None;

                if (currentHand == rightHand)
                {
                    hand = VRTK_DeviceFinder.GetControllerRightHand();
                    handType = SDK_BaseController.ControllerHand.Right;
                }
                else
                {
                    hand = VRTK_DeviceFinder.GetControllerRightHand();
                    handType = SDK_BaseController.ControllerHand.Left;
                }
                VRTK_ControllerEvents tempS = hand.GetComponent<VRTK_ControllerEvents>();
                if (tempS != null)
                {
                    ControllerInteractionEventArgs e = new ControllerInteractionEventArgs
                    {
                        controllerReference = VRTK_ControllerReference.GetControllerReference(handType)
                    };
                    tempS.OnTriggerClicked(e);
                }
            }
            else if (Input.GetKeyUp(triggerAlias))
            {
                GameObject hand = null;
                SDK_BaseController.ControllerHand handType = SDK_BaseController.ControllerHand.None;
